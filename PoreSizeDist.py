...
def separate_pores(binary_image):
    """
    Separates pores using Euclidean distance transform and labels connected pore regions.
...
def main():
    ...
    try:
        ...
        # Step 3: Separate pores and label them
        labeled_pores, num_pores, pore_sizes = separate_pores(binary_image)
        ...
        # Step 6: Save pore sizes if requested
        if output_sizes:
            save_pore_sizes(pore_sizes_for_stats, output_sizes)
        
        # Step 7: Save statistics if requested
        if output_stats:
